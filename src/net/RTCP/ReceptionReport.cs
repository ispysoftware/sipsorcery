...
namespace SIPSorcery.Net
{
    ...
    public class ReceptionReportSample
    {
        ...
        // Properties are used for better encapsulation.
        public uint SSRC { get; private set; }
        public byte FractionLost { get; private set; }
        public int PacketsLost { get; private set; } // This is a 24-bit signed integer.
        public uint ExtendedHighestSequenceNumber { get; private set; }
        public uint Jitter { get; private set; }
        public uint LastSenderReportTimestamp { get; private set; }
        public uint DelaySinceLastSenderReport { get; private set; }

        /// <summary>
        /// Constructor for building a new report sample from data.
        /// </summary>
        public ReceptionReportSample(
            uint ssrc, byte fractionLost, int packetsLost, uint highestSeqNum,
            uint jitter, uint lastSRTimestamp, uint delaySinceLastSR)
        {
            SSRC = ssrc;
        ...
        /// <summary>
        /// High-performance parsing constructor.
        /// </summary>
        public ReceptionReportSample(ReadOnlySpan<byte> buffer)
        {
            if (buffer.Length < PAYLOAD_SIZE)
            {
                throw new ArgumentException("Buffer is too small for a Reception Report Sample.");
            }

            SSRC = BinaryPrimitives.ReadUInt32BigEndian(buffer);
            FractionLost = buffer[4];

            // Efficiently read the 24-bit signed integer for PacketsLost.
            uint packetsLostVal = (uint)((buffer[5] << 16) | (buffer[6] << 8) | buffer[7]);
            // Sign-extend if the 24th bit is set.
            if ((packetsLostVal & 0x800000) > 0)
            {
                PacketsLost = (int)(packetsLostVal | 0xFF000000);
            }
            else
            {
                PacketsLost = (int)packetsLostVal;
            }

            ExtendedHighestSequenceNumber = BinaryPrimitives.ReadUInt32BigEndian(buffer.Slice(8));
            Jitter = BinaryPrimitives.ReadUInt32BigEndian(buffer.Slice(12));
            LastSenderReportTimestamp = BinaryPrimitives.ReadUInt32BigEndian(buffer.Slice(16));
            DelaySinceLastSenderReport = BinaryPrimitives.ReadUInt32BigEndian(buffer.Slice(20));
        }
        ...
        {
            if (destination.Length < PAYLOAD_SIZE)
            {
            ...
            // Efficiently write the 24-bit signed integer.
            destination[5] = (byte)(PacketsLost >> 16);
            destination[6] = (byte)(PacketsLost >> 8);
            destination[7] = (byte)PacketsLost;

            BinaryPrimitives.WriteUInt32BigEndian(destination.Slice(8), ExtendedHighestSequenceNumber);
            BinaryPrimitives.WriteUInt32BigEndian(destination.Slice(12), Jitter);
            BinaryPrimitives.WriteUInt32BigEndian(destination.Slice(16), LastSenderReportTimestamp);
            BinaryPrimitives.WriteUInt32BigEndian(destination.Slice(20), DelaySinceLastSenderReport);
